class FormManager {
      ...
      if (field.type === "select") {
        ...
        if (Array.isArray(field.options)) {
          field.options.forEach((optionValue) => {
            ...
            checkbox.value = optionValue._id;
            checkbox.id = `${field.name}-${optionValue._id}`;
            ...
            if (this.isEditMode && this.initialValues[field.name]) {
              const selectedValues = this.initialValues[field.name];
              if (selectedValues.includes(optionValue._id)) {
                checkbox.checked = true;
              }
            }

            // Add change event listener for each checkbox
            checkbox.addEventListener("change", (e) => {
              const selectedCheckboxes = Array.from(
                wrapper.querySelectorAll('input[type="checkbox"]:checked')
              ).map((checkbox) => checkbox.value);
              this.proxyData[field.name] = selectedCheckboxes;
            });
          });
        }
  ...
  createSubmitButton() {
    ...
    button.type = "button";
    button.textContent = this.isEditMode ? "Save" : "Create";
